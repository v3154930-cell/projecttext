...
STEPS = [
    ...
    FieldStep(name="ask_blocking_date", question="Введите дату блокировки карточки (ДД.ММ.ГГГГ):", data_key="blocking_date", field_type=FieldType.DATE, validators=[validate_date], post_process=normalize_date),
    FieldStep(name="ask_blocking_reason", question="Введите причину блокировки:", data_key="blocking_reason", field_type=FieldType.TEXT, validators=[lambda a: required(a, "Причина блокировки")]),
    FieldStep(name="ask_lost_profit", question="Введите размер упущенной выгоды в рублях:", data_key="lost_profit", field_type=FieldType.MONEY, validators=[validate_money], post_process=format_money),
...
class SellerClaimBlockingScenario(BaseScenario):
    def __init__(self):
        super().__init__(steps=STEPS, template_path="templates/seller_claim_blocking.txt")
