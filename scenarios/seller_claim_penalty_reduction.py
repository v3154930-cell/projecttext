...
STEPS = [
    ...
    FieldStep(name="ask_original_penalty", question="Введите размер исходного штрафа в рублях:", data_key="original_penalty", field_type=FieldType.MONEY, validators=[validate_money], post_process=format_money),
    FieldStep(name="ask_justification", question="Введите обоснование для снижения штрафа:", data_key="justification", field_type=FieldType.TEXT, validators=[lambda a: required(a, "Обоснование")]),
...
class SellerClaimPenaltyReductionScenario(BaseScenario):
    def __init__(self):
        super().__init__(steps=STEPS, template_path="templates/seller_claim_penalty_reduction.txt")
        self._preview_enabled = True
        self._field_assemblers["passport"] = PASSPORT_ASSEMBLER

    def generate_document(self, template_path=None):
        original_penalty = self.data.get("original_penalty", "0")
        try:
            penalty = float(str(original_penalty).replace(' ', '').replace('₽', '').replace(',', '.'))
            reduced = round(penalty * 0.5, 2)
            self.data["reduced_penalty"] = f"{reduced:.2f}".replace('.', ',')
        except:
            self.data["reduced_penalty"] = "0,00"
